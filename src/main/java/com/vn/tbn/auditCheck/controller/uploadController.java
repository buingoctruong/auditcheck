...
public class uploadController {
	...
	@Autowired
	FeedbackCollectionDataRepository feedbackDataRepository;
	...
	public ResponseEntity<?> trainingData(@RequestBody UploadDTO upload, HttpServletRequest request) {
		...
		if (!trainingStatus) {
			if (discoveryService.deleteAllTrainingData(environmentId, collectionId) == 204) {
				...
				System.out.println("====> feedbackDatas : " + feedbackDatas);
				
				List<CollectionData> collectionDatas = collectionDataRepository.findByCorpusId(corpusId);
				System.out.println("====> collectionDatas : " + collectionDatas);
